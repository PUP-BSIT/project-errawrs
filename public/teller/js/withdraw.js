...
// API configuration is imported from api_config.js
	...
	searchTimeout = setTimeout(async () => {
		if (searchTerm.length > 0) {
			try {
				...
				// Use GET request with query parameters and no body
				const url = `${
					API_ENDPOINTS.TELLER_SEARCH_ACCOUNT
				}?search=${encodeURIComponent(
					searchTerm
				)}&teller_number=${encodeURIComponent(
					tellerInfo.teller_number
				)}`;
					...
					data.accounts.forEach((account) => {
						...
						if (account.status === "closed") {
							...
							div.addEventListener("click", () => {
								showNotification(
									"This account is closed. Please reopen the account to make transactions.",
									true
								);
							});
						} else {
...
function selectAccount(account) {
	...
	selectedAccount.balance = parseFloat(
...
document.addEventListener("click", function (e) {
	if (
		!accountInput.contains(e.target) &&
...
function validateAmount(amount) {
	...
	if (isNaN(amount) || amount <= 0) {
		...
		amountInput.classList.add("error");
		validationMessage.textContent = `Maximum withdrawal amount is ${formatCurrency(
			MAX_WITHDRAW_AMOUNT
		)}`;
		validationMessage.style.display = "block";
		confirmButton.classList.remove("active");
...
function toggleLoading(show, amount = 0) {
	...
	if (show) {
		document.getElementById("processing_amount").textContent =
...
function confirmAmount() {
	...
	document.getElementById("confirm_account_number").textContent =
		selectedAccount.account_number;
	document.getElementById("confirm_account_name").textContent =
		selectedAccount.user.name;
	document.getElementById("confirm_current_balance").textContent =
		formatCurrency(selectedAccount.balance);
	document.getElementById("confirm_amount").textContent =
		formatCurrency(amount);
	document.getElementById("new_balance").textContent =
...
async function submitWithdrawal() {
	...
	try {
		const response = await fetch(API_ENDPOINTS.TELLER_WITHDRAW, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
			},
		...
		const newBalance = parseFloat(
			data.data.new_balance.replace(/[^0-9.-]+/g, "")
		);

		// Update receipt
		document.getElementById("transaction_id").textContent =
			data.data.transaction_id;
		document.getElementById("receipt_account_number").textContent =
			data.data.account_number;
		document.getElementById("receipt_account_name").textContent =
			selectedAccount.user.name;
		document.getElementById("receipt_amount").textContent =
			formatCurrency(amount);
		document.getElementById("receipt_new_balance").textContent =
			formatCurrency(newBalance);
		document.getElementById("transaction_date").textContent =
			data.data.transaction_date;
		document.getElementById("teller_number").textContent =
			data.data.teller_number;
...
async function handleWithdrawal(event) {
	...
	try {
		...
		const accountResponse = await fetch(
			`${API_ENDPOINTS.TELLER_SEARCH_ACCOUNT}?search=${encodeURIComponent(
				accountNumber
