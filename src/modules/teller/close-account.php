...
try {
    ...
    try {
        ...
        if ($account['status'] === 'suspended') {
            throw new Exception('Cannot close a suspended account. Please resolve account status first.');
        }
        ...
        if ($pending_count > 0) {
            throw new Exception('Cannot close account with pending transactions. Please wait for all transactions to complete.');
        }

        // Check if account has zero balance
        if ($account['balance'] > 0) {
            throw new Exception('Cannot close account with remaining balance of ' . number_format($account['balance'], 2));
        }
        
        // Update account status to closed
        $update_sql = "UPDATE account SET status = 'closed' WHERE account_id = ? AND balance = 0";
        ...
        if (mysqli_affected_rows($conn) !== 1) {
            throw new Exception('Failed to close account. Please verify account status and balance.');
        }
        ...
        $transaction_sql = "INSERT INTO transaction (
            ...
            status,
            completed_at
        ) VALUES (?, NULL, ?, 'withdrawal', 'completed', NOW())";
