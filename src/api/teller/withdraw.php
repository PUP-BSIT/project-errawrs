...
try {
    ...
    $description = isset($data['description']) ? trim($data['description']) : 'Withdrawal transaction';

    // Validate input
    if (empty($account_number) || $amount <= 0 || empty($teller_number)) {
        throw new Exception('Account number, amount, and teller number are required.');
    }

    // Validate amount format (max 2 decimal places)
    if (round($amount, 2) != $amount) {
        throw new Exception('Invalid amount format. Amount must have at most 2 decimal places.');
    }
    ...
    try {
        ...
        mysqli_stmt_bind_param($transaction_stmt, "idsi", 
            $account['account_id'],
            $amount,
            $description,
            $teller['teller_id']
